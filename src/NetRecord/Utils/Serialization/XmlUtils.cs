...
public static class XmlUtils
{
    ...
    {
        if (string.IsNullOrEmpty(data)) throw new NetRecordException("No Xml data to deserialize");

        var serializer = new XmlSerializer(typeof(TOut));
        using var stringReader = new StringReader(data);
        using var xmlReader = new XmlTextReader(stringReader);
        return (TOut)(serializer.Deserialize(xmlReader) ?? throw new NetRecordException("Error deserializing XML"));
    }
