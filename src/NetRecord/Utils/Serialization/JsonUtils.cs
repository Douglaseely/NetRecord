...
internal static class JsonUtils
{
    ...
    public static TOut? DeserializeNullableJsonToObject<TOut>(string? data, JsonSerializerOptions options)
    where TOut : class
    {
        if (data is null || string.IsNullOrWhiteSpace(data)) return null;
        try
        {
            return JsonSerializer.Deserialize<TOut>(data, options) ??
                   throw new NetRecordException("Deserialization failed");
    ...
    public static string SerializeObjectToJson<TIn>(TIn? data, JsonSerializerOptions options)
    {
        ...
        try
        {
            var serializedResult = JsonSerializer.Serialize(data, options) ?? throw new NetRecordException("Serialization Failed");
    ...
    public static string SerializeObjectToJson(IEnumerable<NetRecordTransaction>? data, JsonSerializerOptions options)
    {
        if (data is null) throw new NetRecordException("Cannot serialize null object");
        try
        {
