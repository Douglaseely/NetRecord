...
public class RequestCensors
{
    ...
    /// <param name="caseSensitive">Whether to match case sensitively.</param>
    /// <returns>The current Censor object.</returns>
    public RequestCensors CensorQueryParametersByKeys(
    ...
    /// <param name="caseSensitive">Whether to match case sensitively.</param>
    /// <returns>The current Censor object.</returns>
    public RequestCensors CensorPathElementsByPatterns(
    ...
    internal string ApplyBodyParametersCensors(
        string body,
    ...
    )
    {
        if (contentType == null)
            throw new NetRecordException(
                "Cannot determine content type of response body, unable to apply censors."
            );
    ...
    internal IDictionary<string, string> ApplyHeaderCensors(IDictionary<string, string> headers)
    {
        ...
        return _headersToCensor.Count == 0
            ? headers
            : headers.ToDictionary(
                header => header.Key,
                header =>
                    ElementShouldBeCensored(header.Key, _headersToCensor)
                        ? _censorText
                        : header.Value
            );
    }
        ...
        else
        {
            ...
            else
            {
                ...
                foreach (var key in queryParameters.AllKeys)
                {
                    ...
                    censoredQueryParameters.Add(
                        key,
                        ElementShouldBeCensored(key, _queryParamsToCensor)
                            ? _censorText
                            : queryParameters[key]
                    );
                }
    ...
    public static string CensorJsonData(
        string data,
        string censorText,
        IReadOnlyCollection<RequestCensorElement> elementsToCensors,
        JsonSerializerOptions options
    )
    {
        try
        {
            var jsonDictionary = JsonUtils.DeserializeJsonToObject<Dictionary<string, object>>(
                data,
                options
            );
            var censoredJsonDictionary = ApplyDataCensors(
                jsonDictionary,
                censorText,
                elementsToCensors
            );
    ...
    public static string CensorXmlData(
        string data,
        string censorText,
        IReadOnlyCollection<RequestCensorElement> elementsToCensors
    )
    {
        try
        {
            ...
            var censoredXmlDictionary = ApplyDataCensors(
                xmlDictionary,
                censorText,
                elementsToCensors
            );
    ...
    private static List<object> ApplyDataCensors(
        List<object> list,
        string censorText,
        IReadOnlyCollection<RequestCensorElement> elementsToCensors
    )
    {
        ...
        foreach (var entry in list)
        {
            if (entry is Dictionary<string, object> entryDict)
            {
                ...
                {
                    var censoredEntryDict = ApplyDataCensors(
                        entryDict,
                        censorText,
                        elementsToCensors
                    );
            ...
            else if (entry is List<object> entryList)
            {
                ...
                {
                    var censoredEntryList = ApplyDataCensors(
                        entryList,
                        censorText,
                        elementsToCensors
                    );
    ...
    private static Dictionary<string, object> ApplyDataCensors(
    ...
    private static bool ElementShouldBeCensored(
        string foundKey,
        IReadOnlyCollection<RequestCensorElement> elementsToCensor
    )
    {
        return elementsToCensor.Count != 0
    ...
    private static string ToQueryString(NameValueCollection queryParamCollection)
    {
        return string.Join(
            ...
            queryParamCollection
                .AllKeys.Select(key =>
                    $"{HttpUtility.UrlEncode(key)}={HttpUtility.UrlEncode(queryParamCollection.Get(key))}"
                )
                .ToArray()
        );
    }
